...
By the gurobi optimization process we have an upper and lower bound for any optimization problem given, including vaghar for a network $N$: $\delta_l^{vaghar}(N) \leq \delta^{vaghar}(N) \leq \delta_u^{vaghar}(N)$
\end{definition}

\begin{claim}\label{claim:x2_eps_N1}
for the following notions:
\begin{itemize}
    \item By definition ~\ref{def:delta_u} we can say that for $\delta^\text{max}_{\delta_p}(N_1)\ \exists{x^1_{\delta_p},\epsilon^1_{\delta_p}}$ such that
    $$\mathcal{C}(x^1_{\delta_p},c,N_1) = \delta^\text{max}_{\delta_p}(N_1),\ \mathcal{C}(f_P(x^1_{\delta_p},\epsilon^1_{\delta_p}),c,N_1)\leq \delta_p$$
    \item By definition ~\ref{def:vhagar} we can say that for $\delta^{vaghar}(N_2)\ \exists{x^2_{vaghar},\epsilon^2_{vaghar}}$ such that
    $$\mathcal{C}(x^2_{vaghar},c,N_2) = \delta^{vaghar}(N_2),\ \mathcal{C}(f_P(x^2_{vaghar},\epsilon^2_{vaghar}),c,N_1)\leq 0$$
  \end{itemize}
  we can say:
...
For $\delta_p=\delta_{diff}$ we can say:
$$\forall x.\ |\mathcal{C}(x,c,N_2)-\mathcal{C}(x,c,N_1)|\leq \delta_p$$
this is also true for $f_P(x^2_{vaghar},\epsilon^2_{vaghar})$:
...
By definition ~\ref{def:vhagar} we can say that $ 0 \leq -\mathcal{C}(f_P(x^2_{vaghar},\epsilon^2_{vaghar}),c,N_2) $ so:
...
Given 2 networks $N_1,N_2$ where $\forall x.\ |\mathcal{C}(x,c,N_2)-\mathcal{C}(x,c,N_1)|\leq \delta_{diff}$ we can say $ \delta^\text{max}_{\delta_p}(N_1)+\delta_{diff} \geq \delta_u^{vaghar}(N_2)$
\end{claim}
