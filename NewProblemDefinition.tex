...
\cite{chen2014small,he2019streaming}, and \emph{autonomous vehicles} require on-board perception models to react in real time, since relying solely on cloud connectivity could introduce
...
their full-precision counterparts \cite{banner2019post}. This tension
motivates the design of \textbf{cloud–edge hybrid frameworks}, where a
...
questions of \textbf{robustness}.
Quantized neural networks (QNNs) remain vulnerable to adversarial perturbations, when even a small,
...
Studies show that adversarial examples crafted for full-precision models often
transfer to their quantized versions, indicating that quantization alone does
not provide robustness~\cite{zhao2019adversarial}.

This vulnerability extends to \emph{cloud--edge hybrid frameworks}, where a
quantized model runs locally and a full-precision model operates in the cloud.
...
\begin{comment}
Recent works have begun to investigate robustness in hybrid scenarios. For
instance, Ding et al.~\cite{ding2024hybridllm} explored hybrid query routing
between smaller and larger language models, suggesting the importance of
robustness-aware routing. Similarly, Hao~\cite{hao2024hybrid} studied
edge--cloud collaborative inference, which highlights open challenges in
ensuring adversarial robustness when combining quantized edge models with
cloud-based full-precision models. Nevertheless, a comprehensive defense
strategy that jointly protects both edge and cloud components against
perturbation-based attacks remains an open research direction.
\end{comment}
...
Building upon these foundations, more recent work has sought to improve efficiency and scalability. For example, \cite{huang2023towards} presents an integer linear programming (ILP) framework with soundness and completeness guarantees, enhanced by heuristic strategies.  However, all of those works refer to local robustness of a single QNN network, with no consideration of the larger NN.
...
In this work, we present a novel approach to verifying the robustness of hybrid cloud–edge systems against adversarial attacks, where a full-precision neural network resides in the cloud and its quantized counterpart operates on the edge, with each query being processed by only one of the two models. We consider the routing between the cloud and edge to be errorless \Neta{rephase!!}
