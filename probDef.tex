...
Given two classifiers $D_1,D_2: \mathbb{R}^{n \times m} \rightarrow {\mathbb{R}}^{|C|}$, a class $c\in{C}$, a perturbation $P$ and its range $I_\epsilon$, our goal is to compute the maximal class confidence $\delta$, where there $\exists{x}\exists{\epsilon'}\in{I_\epsilon}$ satisfying:\Dana{please explain in words this definition}

\begin{enumerate}[nosep,nolistsep]
    \item $\mathcal{C}_{F_1}(x,c',D) \geq \delta$\Dana{what is D? should it be $D_1$?}
