...
Given two classifiers $F_1,F_2: \mathbb{R}^{n \times m} \rightarrow {\mathbb{R}}^{|C|}$, a class $c\in{C}$, a perturbation $P$ and its range $I_\epsilon$, our goal is to compute the maximal class confidence $\delta$, where there $\exists{x}\exists{\epsilon'}\in{I_\epsilon}$ satisfying:%\Dana{please explain in words this definition}
...
\end{enumerate} 
In simple terms, the class confidence score of $F_1$ for class $c'$ is maximized so that $F_2$ also classifies $x$ as $c'$, while both classifiers $F_1$ and $F_2$ fail to classify the perturbed input $f_P(x,\epsilon')$. 