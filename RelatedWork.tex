...
ReluDiff~\cite{RELUDIFF} computes the maximal difference of the output of two closely related neural networks\Dana{is it in a given neighborhood? we should say this is local if yes}, differing only in the values of their weights. However, it suffers from severe error-explosion. NeuroDiff~\cite{NEURODIFF} relies on a symbolic and fine-grained approximation technique to reduce this approximation error.
...
QEBVerif~\cite{QEBVERIF} examines the relationship between a quantized network and an original network\Dana{a global or a local property?}. QEBVerif determines whether the quantization error of a DNN remains within a specified bound. 
...
\Dana{put the citation number at the end of the sentence (this repeats several times)}\cite{CFXROBUSTNESS} introduces a verification method for local robustness of counterfactual explanations (CFXs).
