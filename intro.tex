...
To make them reliable, many verifiers have been developed to verify various kinds of properties, such as adhering to constraints~\cite{Reluplex,COMPLETE,PLANET} %\Dana{add 2 more citations} 
or being locally robust to attacks~\cite{Reluplex,ABSTRACTINTER,NEURIPS2021_fac7fead,COMPLETE}.%\Dana{add one more citations}. 
%Existing verifiers take non-negligible time. %and often verify local properties, which do not generalize to any input. 
However, similarly to classical software, neural networks are often evolving to increase their accuracy or extend their function to new kinds of inputs ~\cite{SURVEY_ROBUSTNESS,INTPROP}. %\Dana{find citations}
While it is possible to run from scratch the existing verifiers, this introduces high overhead.
This gave rise to incremental verification ~\cite{IVAN,INC_SEMIDEFINIT,INC_SMOOTH}.%\Dana{add citations}.  
...
We extended a global robustness verifier for a single network ~\cite{VHAGAR,MEASURENNROBCON,GLOBALPROPERTY}
%\Dana{add citation}
