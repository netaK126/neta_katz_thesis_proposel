...
To make them reliable, many verifiers have been developed to verify various kinds of properties, such as adhering to constraints~\cite{Reluplex}\Dana{add 2 more citations} or being locally robust to attacks~\cite{Reluplex,ABSTRACTINTER}\Dana{add one more citations}. 
%Existing verifiers take non-negligible time. %and often verify local properties, which do not generalize to any input. 
However, similarly to classical software, neural networks are often evolving to increase their accuracy or extend their function to new kinds of inputs\Dana{find citations}. 
While it is possible to run from scratch the existing verifiers, this introduces high overhead.
This gave rise to incremental verification \Dana{add citations}.  
...
Differential analysis focuses on determining the differences of two similar programs\Dana{add 4-5 citations}. 
...
making this problem highly complex. Further, although there are many verifiers for neural networks, the vast majority focuses on local properties\Dana{add 6-7 citations}, and thus cannot be extended to our global property.
While there are global robustness verifiers for neural networks\Dana{add all citations}, they all focus on analyzing a single network. 
...
We extended a global robustness verifier for a single network\Dana{add citation}
